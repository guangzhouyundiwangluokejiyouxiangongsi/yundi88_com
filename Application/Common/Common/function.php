 ...
 * 关键词加红
 */
function zfgetsubstr($string, $start, $length, $q){

    if(mb_strlen($string,'utf-8')>$length){
          $str = mb_substr($string, $start, $length,'utf-8');
          $str = str_replace($q, "<zhoufei class='myq'>{$q}</zhoufei>", $str);
          return $str.'...';
      }else{
            $str = str_replace($q, "<zhoufei class='myq'>{$q}</zhoufei>", $string);
          return $str;
      }
...
function myget_images($url){
     $arr = explode('/', $url);
     $count = count($arr);
     $filename = $arr[$count-1];
     $savefile = '.'.str_replace($filename,'',str_replace('/Public', '/Public/temporary',$url));
     $url = 'http://template.yundi88.com'.$url;
     $url2 = 'http://www.yundi88.com'.$url;
    if (file_exists($url2)){
        return false;
    }
...
// $str = file_get_contents('http://template.yundi88.com'.$path);
// $path2 = str_replace('/Public','/Public/temporary', $path);
// // if(!is_dir($path2)){mkdir('.'.$path2, 0777,true);}
// // return $path2;
// // file_put_contents('.'.$path2, $str);
// file_put_contents('./Public/temporary/upload/ad/2017/03-32/58dde4a29fd9dwwww.jpg', $str);
...
function sp_getcontent_imgs2($content){
   
            $s = preg_match_all('/\/Public.*?\.(jpg)|(png)|(gif)|(bmp)$/',$content,$imgs);
            $data[] = $imgs[0];
            return $data;      

}

function sp_getcontent_imgs3($content){
