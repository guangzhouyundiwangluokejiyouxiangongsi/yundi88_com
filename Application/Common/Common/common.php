...
function sendSMS2($mobile, $code,$time)
{
    //时区设置：亚洲/上海
    date_default_timezone_set('Asia/Shanghai');
    //这个是你下面实例化的类
    ...
    $req->setRecNum("$mobile");
    //短信模板ID，传入的模板必须是在阿里大鱼“管理中心-短信模板管理”中的可用模板。
    $req->setSmsTemplateCode($config['sms_templateCode2']); // templateCode
...
function sendSMS3($mobile, $code ,$time)
{
    //时区设置：亚洲/上海
    date_default_timezone_set('Asia/Shanghai');
    //这个是你下面实例化的类
    ...
    $req->setRecNum("$mobile");
    //短信模板ID，传入的模板必须是在阿里大鱼“管理中心-短信模板管理”中的可用模板。
    $req->setSmsTemplateCode($config['sms_templateCode3']); // templateCode
...
function sendSMS4($mobile, $code,$time)
{
    //时区设置：亚洲/上海
    date_default_timezone_set('Asia/Shanghai');
    //这个是你下面实例化的类
    ...
    $req->setRecNum("$mobile");
    //短信模板ID，传入的模板必须是在阿里大鱼“管理中心-短信模板管理”中的可用模板。
    $req->setSmsTemplateCode($config['sms_templateCode4']); // templateCode
...
function news_thum_images($news_id,$width,$height){
// /Public/upload/seller/2016/12-19/585782959fe58.jpg
     if(empty($news_id)) return '';
    //判断缩略图是否存在
    $path = "Public/upload/news/seller/$news_id/";
    $news_thumb_name ="news_thumb_{$news_id}_{$width}_{$height}";
  
    // 这个商品 已经生成过这个比例的图片就直接返回了
    if(file_exists($path.$news_thumb_name.'.jpg'))  return '/'.$path.$news_thumb_name.'.jpg'; 
    if(file_exists($path.$news_thumb_name.'.jpeg')) return '/'.$path.$news_thumb_name.'.jpeg'; 
    if(file_exists($path.$news_thumb_name.'.gif'))  return '/'.$path.$news_thumb_name.'.gif'; 
    if(file_exists($path.$news_thumb_name.'.png'))  return '/'.$path.$news_thumb_name.'.png'; 
        
    $original_img = M('store_art')->where("id = $news_id")->getField('newsimg');
    if(empty($original_img)) return '';
    ...
    try{
        $image = new \Think\Image();
        $image->open($original_img);        
        $news_thumb_name = $news_thumb_name. '.'.$image->type();
        // 生成缩略图
        if(!is_dir($path)) mkdir($path,0777,true);      
        // 参考文章 http://www.mb5u.com/biancheng/php/php_84533.html  改动参考 http://www.thinkphp.cn/topic/13542.html
        $image->thumb($width, $height,2)->save($path.$news_thumb_name,NULL,100); //按照原图的比例生成一个最大为$width*$height的缩略图并保存
        //图片水印处理
        $water = tpCache('water');
        if($water['is_mark']==1){
            $imgresource = './'.$path.$news_thumb_name;
            if($width>$water['mark_width'] && $height>$water['mark_height']){
                if($water['mark_type'] == 'img'){
                    $image->open($imgresource)->water(".".$water['mark_img'],$water['sel'],$water['mark_degree'])->save($imgresource);
                }else{
