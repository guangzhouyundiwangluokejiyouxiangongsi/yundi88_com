...
//        $show = $page->show();
    ...
    {   
        $cat_id = I('id',1);
        $cat_id = $this->putgetgoods($cat_id);
        $cat_id = $this->putgetgoods($cat_id);
        $category = M('goods_category')->where(array('id'=>$cat_id))->find();
        $this->assign('infolist',$category);
        $this->assign('cat_name',$category['name']);
    ...
    public function putgetgoods($cat_id=1,$p=1,$num=1,$order='')
    {   
        ...
        $map['home_is_show'] = 1;
        $map['goods_state'] = 1;
        $goods_list = M('goods')->field('goods_id')->where($map)->find();
        if (!$goods_list && $p<2){
            $cat_id = M('goods_category')->where(array('id'=>$cat_id))->getField('parent_id');
            return $cat_id;
        }
        return $cat_id;
    }
