...
class ArticleController extends BaseController {
    ...
    public function article(){
        ...
        if(I('GET.article_id')){
           ...
           // if ($info['is_new_open'] == 1){
           //  $info['link'] = substr($info['link'],0,-16);
           // }
        }
    ...
    public function aticleHandle(){
        $data = I('post.');
        // if ($data['is_new_open'] == 1){
        //     $data['link'] = $data['link'].' "target="_blank';
        // }
    ...
    public function shopHandle(){
        $data = I('post.');
        if($data['act'] == 'add'){
            if (!$data['link_name']) $this->error('链接名字不能为空');
            $r = M('shop_link')->add($data);
        }
        if($data['act'] == 'edit'){
            if (!$data['link_name']) $this->error('链接名字不能为空');
            $r = M('shop_link')->where('link_id='.$data['link_id'])->save($data);
        }
        
        if($data['act'] == 'del'){
            $r = M('shop_link')->where('link_id='.$data['link_id'])->delete();
            if($r) exit(json_encode(1));
        }
        
        if($r){
            $this->success("操作成功",U('Admin/Article/shopList'));
        }else{
            $this->error("操作失败",U('Admin/Article/shopList'));
        }
    }
    ...
    public function shoplink(){
        ...
        $link_info = array();
        if($link_id){
            $link_info = M('shop_link')->where('link_id='.$link_id)->find();
            $this->assign('info',$link_info);
        }
        $this->display();
    }
