...
class UsersLogic extends RelationModel
{
    ...
    public function login($username,$password){
        ...
        if (!$user && $seller){
            $user = M('users')->where(array('user_id'=>$seller['user_id']))->find();
        }
        if(!$user){
...
//        	}
    ...
    public function add_address($user_id,$address_id=0,$data){
        ...
        if ($post['province'] != 47493 && $post['province'] != 47494 && $post['province'] != 47495){
     ...
     * @return json
     */
    public function send_validate_code2($sender,$type){
    	$sms_time_out = tpCache('sms.sms_time_out');
    	$sms_time_out = $sms_time_out ? $sms_time_out : 120;
    	//获取上一次的发送时间
    	...
    	if($type == 'email'){
            ...
            if (M('users')->where(array('mobile'=>$sender))->find()){
                $name = $sender;
                $time = date('Y-m-d H:i:s',time());
               $send = sendSMS4($name,$code,$time);
            } else {
    ...
    public function send_validate_code($sender,$type){
        ...
        if($type == 'email'){
            ...
            //$send = sendSMS($sender,'您好，你的验证码是：'.$code);
                $time = date('Y-m-d H:i:s',time());
                $name = $sender;
                $user = M('users')->where(array('mobile'=>$sender))->find();
                if ($user){
                    $send = sendSMS3($name,$code,$time);
                } else {
                    $send = sendSMS2($name,$code,$time);
                }
        }
        if($send){
            $info['code'] = $code;
