...
//        	}
    ...
    public function add_address($user_id,$address_id=0,$data){
        ...
        if(!check_mobile($post['mobile']) && (!preg_match('/^((\d{3,4}-)|\d{3.4}-)?\d{7,8}$/',$post['mobile'])))
            return array('status'=>-1,'msg'=>'手机号码或固定电话格式有误','result'=>'');
    ...
    public function send_validate_code($sender,$type){
    	...
    	if(!empty($send) && $send['time'] > time() && $send['sender'] == $sender){
    		//在有效期范围内 相同号码不再发送
    		$res = array('status'=>-1,'msg'=>'两分钟内不能重复发送验证码');
            return $res;
    	}
    	...
    	if($send){
    		...
    		$res = array('status'=>-1,'msg'=>'验证码发送频繁,请稍等两分钟重新点击');
    	}
    	return $res;
    }
