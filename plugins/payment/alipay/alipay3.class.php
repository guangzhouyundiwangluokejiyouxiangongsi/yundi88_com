...
{    
    ...
    {        
            ...
            $parameter = array(
                        ...
                        "notify_url"	=> SITE_URL.U('Seller/Commerce/notifyUrl',array('pay_code'=>'alipay')) , //服务器异步通知页面路径 //必填，不能修改
                        "return_url"	=> SITE_URL.U('Seller/Commerce/returnUrl',array('pay_code'=>'alipay')),  //页面跳转同步通知页面路径
                        "out_trade_no"	=> $order['order_sn'], //商户订单号                        
                        "subject"	=> '加入云狄网云商会', //订单名称 可以中文
    ...
    {                
        ...
        $verify_result = $alipayNotify->verifyNotify();
            ...
            if($verify_result) //验证成功
            {
                    ...
                    {                         
                        // M('domain')->where(array('order'=>$order_sn))->save(array('status'=>1)); // 修改订单支付状态
                        M('records')->where(array('order_id'=>"{$order_sn}"))->save(array('status'=>1));
                        // M('store')->where(array('store_id'=>$order['store_id']))->save(array('commerce_state'=>1));
                    }
